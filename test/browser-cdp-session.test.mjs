...
import { __test, closeBrowserSession } from '../lib/tools/browser-cdp-session.mjs';
...
describe('browser CDP live session helpers', () => {
  ...
  it('streams real browser frames over SSE and dispatches input through CDP', async () => {
    await withLiveBrowserServer(async (baseUrl) => {
      ...
      const opened = await browserActionTool.handler({ action: 'open', url: `${baseUrl}/cdp-live-test` }, {
        config: { ...browserActionTool.defaultConfig, allowedHosts: [] },
        emit() {},
      });
      ...
      await browserActionTool.handler({ action: 'click', selector: '#name' }, {
        config: { ...browserActionTool.defaultConfig, allowedHosts: [] },
        emit() {},
      });
      ...
      const typed = await browserActionTool.handler({
        action: 'evaluate',
