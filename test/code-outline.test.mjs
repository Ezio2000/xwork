...
import { codeOutlineTool } from '../lib/tools/builtin/code-outline.mjs';
...
describe('code_outline tool', () => {
  ...
  it('is registered and enabled by default', async () => {
    ...
    const tool = tools.find(t => t.id === 'code_outline');
    assert.ok(tool);
    assert.equal(tool.dangerLevel, 'low');
    assert.equal(codeOutlineTool.defaultEnabled, true);
  });
