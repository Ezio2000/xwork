...
import { askUserTool, normalizeAskUserInput } from '../lib/tools/builtin/ask-user.mjs';
...
import { RUN_EVENT_TYPES } from '../lib/run-events.mjs';
...
describe('ask_user tool', () => {
  it('is registered and enabled by default', async () => {
    ...
    const tool = tools.find(t => t.id === 'ask_user');
    assert.ok(tool);
    assert.equal(tool.enabled, true);
    assert.equal(askUserTool.defaultEnabled, true);
  });
  ...
  it('includes aggressive ask_user guidance in system prompt', () => {
    const system = buildSystemPrompt([], { tools: [askUserTool] });
