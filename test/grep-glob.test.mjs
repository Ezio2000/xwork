...
import { grepTool } from '../lib/tools/builtin/grep.mjs';
import { globTool } from '../lib/tools/builtin/glob.mjs';
import {
  buildSearchRegex,
  grepWorkspaceFiles,
...
describe('glob tool', () => {
  ...
  it('returns glob-list render data', async () => {
    await withToolEnabled('glob', globTool, async () => {
      ...
      assert.ok(result.output.files.some(file => file.path.endsWith('grep.mjs')));
    });
  });
