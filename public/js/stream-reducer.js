...
function applyToolResult(evt, stream, effects) {
  for (const tool of evt.tools) {
    ...
    if (tool.renderType && tool.data) {
      ...
      if (block.type === 'source-cards' || block.type === 'sources' || block.type === 'web-fetch' || block.type === 'file-snippet' || block.type === 'shell-command' || block.type === 'mysql-query' || block.type === 'sqlite-query') block.collapsed = true;
