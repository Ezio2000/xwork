...
import { getWorkspaceRoot, isInsideWorkspace, onWorkspaceChange } from './workspace-root.mjs';

export { getWorkspaceRoot } from './workspace-root.mjs';
...
export const DEFAULT_LIST_DIR_DEPTH = 2;
export const MAX_LIST_DIR_DEPTH = 5;
export const DEFAULT_LIST_DIR_LIMIT = 200;
export const MAX_LIST_DIR_LIMIT = 500;
...
export async function resolveWorkspaceDirectoryPath(path = '.') {
  const inputPath = path === undefined || path === null || path === '' ? '.' : path;
  requiredString(String(inputPath), 'path', MAX_PATH_LENGTH);
  ...
  const dirStat = await stat(canonical);
...
async function collectDirectoryEntries(dirAbs, dirRel, currentDepth, maxDepth, options, entries, state) {
  ...
  let dirEntries;
  try {
    dirEntries = await readdir(dirAbs, { withFileTypes: true });
  } catch {
    return;
  }
  ...
  for (const entry of dirEntries) {
    ...
    const entryRel = dirRel && dirRel !== '.'
      ? `${dirRel}/${name}`.replace(/\\/g, '/')
      : name;
    const entryAbs = join(dirAbs, name);
    const query = options.query;

    if (entry.isDirectory()) {
      const skipped = SKIP_DIR_NAMES.has(name);
      const policy = getPathPolicyFailure(entryRel, name, { blockedGlobs: options.blockedGlobs });
      const blocked = policy?.code === 'blocked_path';
      if (query && !entryRel.toLowerCase().includes(query) && !name.toLowerCase().includes(query)) {
        if (!skipped && !blocked && currentDepth < maxDepth) {
          await collectDirectoryEntries(entryAbs, entryRel, currentDepth + 1, maxDepth, options, entries, state);
        }
        continue;
      }
...
export async function listWorkspaceDirectory({
  ...
  const { absolutePath, relativePath } = await resolveWorkspaceDirectoryPath(path);
  const normalizedDepth = clampInteger(depth, DEFAULT_LIST_DIR_DEPTH, 1, MAX_LIST_DIR_DEPTH);
  const normalizedLimit = clampInteger(limit, DEFAULT_LIST_DIR_LIMIT, 1, MAX_LIST_DIR_LIMIT);
...
async function getFileIndex({ blockedGlobs = DEFAULT_BLOCKED_GLOBS, force = false } = {}) {
  ...
  const files = [];
  await walkWorkspaceFiles(getWorkspaceRoot(), '', 0, files, blockedGlobs);
...
onWorkspaceChange(() => invalidateWorkspaceFileIndex());
