...
export const DEFAULT_BASE_PRICING = [
  {
    ...
    notes: 'DeepSeek official public pricing. Web search request pricing is not listed on the model pricing page. Fixed per-call API pricing defaults to 0 unless configured.',
  },
  {
    id: 'deepseek-v4-pro',
    provider: 'deepseek',
    ...
    notes: 'DeepSeek v4 pro 75% promotional pricing shown on the official pricing page as of 2026-05-17. Fixed per-call API pricing defaults to 0 unless configured.',
  },
];
...
function costFor({ count, pricing, field, perRequest = false, defaultPrice = null }) {
  if (count <= 0) return { cost: 0, missing: false };
  const rawPrice = pricing?.[field];
  const price = rawPrice === undefined || rawPrice === null || rawPrice === '' ? defaultPrice : rawPrice;
...
export function calculateUsageCost(usage, effectivePricing) {
  ...
  return {
    ...
    requestCost: parts.requestCost.missing ? null : parts.requestCost.cost,
    totalCost,
    missingFields,
    unpricedUsage,
  };
