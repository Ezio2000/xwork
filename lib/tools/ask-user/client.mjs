...
export function installHandlers(root) {
  ...
  root.addEventListener('click', async (event) => {
    ...
    if (answerBtn) {
      ...
      try {
        markAskUserFormSubmitting(form);
        await submitAskUserInput(runId, toolCallId, {
          status: 'answered',
          answer: answerBtn.dataset.askValue || answerBtn.textContent.trim(),
        });
      } catch (err) {
        markAskUserFormError(form, err.message || String(err));
      }
    ...
    if (skipBtn) {
      ...
      if (!runId || !toolCallId) return;
      try {
        markAskUserFormSubmitting(form, '已跳过…');
        await submitAskUserInput(runId, toolCallId, { status: 'skipped' });
      } catch (err) {
        markAskUserFormError(form, err.message || String(err));
      }
    }
