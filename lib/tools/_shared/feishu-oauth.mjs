import { patchStoredToolConfig, readStoredToolConfig } from '../_core/store.mjs';
...
export const FEISHU_DOMAIN_SCOPES = {
  ...
  media: ['docs:document.media:download'],
...
export async function loadFeishuAuthConfig(config = {}, { includeStoredAuth = true, includeStoredRead = true, authOverridesConfig = false } = {}) {
  ...
  if (authOverridesConfig) return mergeConfig(readStored, config, authStored);
...
export function configWithOAuthScopes(config = {}, ...extraScopes) {
  const base = mergeOAuthScopes(
    configuredString(config, 'oauthScope', DEFAULT_FEISHU_OAUTH_SCOPE),
    configuredString(config, 'oauthScope_user_authorized', ''),
  );
  return {
    ...config,
    oauthScope: mergeOAuthScopes(base, ...extraScopes),
  };
}
...
export function shouldStartUserAuthorization(err) {
  ...
  return /user_access_token is not configured|invalid.*token|token.*invalid|expired.*token|token.*expired|unauthorized|access token|access_token|invalid_auth|auth.*failed|no.?permission|permission.?denied|access.?denied|20027|99991663|99991664|99991667|99991668/.test(text);
}
...
export async function saveUserToken(tokenData, oauthScope = '') {
  const scopeToSave = String(oauthScope || '').trim();
  const patch = currentConfig => ({
    ...
    refresh_token_expires_at: tokenData.refreshExpiresIn
      ...
      : '',
    ...(scopeToSave ? { oauthScope_user_authorized: scopeToSave } : {}),
  });
...
export async function clearStoredUserTokens() {
  const patch = currentConfig => {
    const next = { ...currentConfig };
    for (const key of TOKEN_CONFIG_KEYS) delete next[key];
    next.user_access_token = '';
  ...
  await patchStoredToolConfig('feishu_auth', patch);
  await patchStoredToolConfig('feishu_read', patch);
}
...
export async function pollDeviceAuthorization(config = {}, auth, { signal, emit, action = 'login' } = {}) {
  ...
  while (Date.now() < expiresAt) {
    try {
      ...
      await saveUserToken(tokenData, scope);
      emit?.({
...
export async function completeDeviceAuthorization(input = {}, { config = {}, signal } = {}) {
  ...
  await saveUserToken(tokenData, configuredOAuthScope(config));
  return {
