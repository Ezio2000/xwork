...
export const mysqlQueryTool = {
  ...
  configSchema: {
    ...
    properties: {
      sources: {
        type: 'array',
        description: 'Configured MySQL sources. Each source supports id, host or ip, port, username, password, and optional database.',
        items: {
          type: 'object',
          properties: {
            id: { type: 'string' },
      ...
      connectTimeoutMs: {
        type: 'number',
        description: 'MySQL connection timeout and query max execution time in milliseconds.',
      },
    },
