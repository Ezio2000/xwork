...
export const webSearchTool = {
  ...
  async handler(input, { config, context, signal }) {
    ...
    try {
      ...
      throw new Error(`Web search returned non-JSON response: ${respText.slice(0, 200)}`);
    }
    ...
    if (!sources.length && !errors.length) {
      throw new Error(`Web search returned no results. Response: ${respText.slice(0, 300)}`);
    }
