function systemPrompt() {
  return [
    ...
    `- CRITICAL: while generating this single assistant response right now, you may call web_search at most ${maxUses} times. Count every web_search call you make in this response yourself.`,
    `- The ${maxUses}-call limit applies only to the current response you are generating. Do not interpret it as a total limit for the whole conversation, and do not assume a hidden counter will protect you.`,
    `- Once you have made ${maxUses} web_search calls in this response, stop using web_search immediately and answer with the results already collected. For straightforward questions, use at most 1-2 searches.`,
