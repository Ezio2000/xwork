function systemPrompt() {
  return [
    ...
    `- CRITICAL: web_search has a HARD LIMIT of ${maxUses} uses total for this entire conversation. You MUST NOT exceed ${maxUses} calls under any circumstances. Use at most 1-2 searches for straightforward questions. If you ever receive a "max_uses_exceeded" error, you have hit the hard limit: you MUST stop immediately, you MUST NOT retry web_search, and you MUST answer using only the results already collected. Calling web_search again after max_uses_exceeded will fail and waste a turn.`,
