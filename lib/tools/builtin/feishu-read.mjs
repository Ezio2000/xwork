...
function hasConfiguredUserAccessToken(config = {}) {
  return Boolean(
    configuredString(config, 'user_access_token')
    || configuredString(config, 'userAccessToken')
...
async function feishuResourceApi(config, path, options = {}) {
  ...
  const effectiveConfig = await loadFeishuAuthConfig(config || {}, { authOverridesConfig: true });
  ...
  async function requestAsUser(currentConfig) {
    let accessToken;
    try {
      accessToken = getUserAccessToken(currentConfig);
    } catch (tokenErr) {
      if (!shouldStartUserAuthorization(tokenErr)) throw tokenErr;
      const { tokenData } = await authorizeAndWait(currentConfig, { signal, emit, action: authAction });
      accessToken = tokenData.accessToken;
    }

    try {
      const payload = await requestJson(feishuBaseUrl(currentConfig), path, { ...requestOptions, signal, accessToken });
      return { payload, config: { ...currentConfig, user_access_token: accessToken, forceUserAccessToken: 'true' }, authorizedAsUser: true };
    } catch (userErr) {
      if (!shouldStartUserAuthorization(userErr)) throw userErr;
      const { tokenData } = await authorizeAndWait(currentConfig, { signal, emit, action: authAction });
      const payload = await requestJson(feishuBaseUrl(currentConfig), path, { ...requestOptions, signal, accessToken: tokenData.accessToken });
      return { payload, config: { ...currentConfig, user_access_token: tokenData.accessToken, forceUserAccessToken: 'true' }, authorizedAsUser: true };
    }
  }

  if (configuredString(effectiveConfig, 'forceUserAccessToken', '') === 'true' || hasConfiguredUserAccessToken(effectiveConfig)) {
    return requestAsUser(authConfig);
  }

  try {
    return await feishuApi(effectiveConfig, path, { ...requestOptions, signal });
  } catch (err) {
...
export const feishuReadTool = {
  ...
  systemPrompt() {
    return [
      ...
      '- If a feishu_read call fails for a Feishu URL because credentials, user_access_token, permission, scope, authorization, or access token is missing/invalid, do not retry the same URL with different parameters. Use the built-in authorization flow and wait for that tool result.',
