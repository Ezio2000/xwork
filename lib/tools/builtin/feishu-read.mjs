...
async function getWikiNode(input, { config, signal }) {
  ...
  const payload = await feishuApi(config, '/open-apis/wiki/v2/spaces/get_node', {
    query: {
      token,
...
export const feishuReadTool = {
  ...
  configSchema: {
    ...
    properties: {
      ...
      oauthScope: { type: 'string', description: 'OAuth scopes requested by shared feishu_auth Device Flow when auto-authorizing current user. Defaults to auth:user.id:read.' },
