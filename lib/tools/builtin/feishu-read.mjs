...
const DEFAULT_AUTH_BASE_URL = DEFAULT_FEISHU_AUTH_BASE_URL;
...
const DEFAULT_OAUTH_SCOPE = DEFAULT_FEISHU_OAUTH_SCOPE;
...
function shouldAuthorizeForResource(err) {
  const text = [
    err?.message,
    err?.code,
    err?.payload?.msg,
  ...
  return /tenant needs read permission|permission denied|permission.*denied|no permission|forbidden|unauthorized|scope|access denied|99991663|99991668|20027/.test(text);
}

async function feishuResourceApi(config, path, options = {}) {
  const {
    signal,
    emit,
    authAction = 'read_resource',
    authDomains = [],
    authScopes = [],
    ...requestOptions
  } = options;
  if (configuredString(config, 'forceUserAccessToken', '') === 'true') {
    const accessToken = getUserAccessToken(config);
    const payload = await requestJson(feishuBaseUrl(config), path, { ...requestOptions, signal, accessToken });
    return { payload, config, authorizedAsUser: true };
  }
  try {
    return await feishuApi(config, path, { ...requestOptions, signal });
  } catch (err) {
    if (!configuredBoolean(config, 'autoAuthorizeResources', true) || !shouldAuthorizeForResource(err)) throw err;
    const authConfig = configWithOAuthScopes(
      await loadFeishuAuthConfig(config || {}, { includeStoredAuth: false }),
      scopesForDomains(authDomains),
      authScopes,
    );
    let accessToken;
    try {
      accessToken = getUserAccessToken(authConfig);
    } catch (tokenErr) {
      if (!shouldStartUserAuthorization(tokenErr)) throw err;
      const { tokenData } = await authorizeAndWait(authConfig, { signal, emit, action: authAction });
      accessToken = tokenData.accessToken;
    }

    try {
      const payload = await requestJson(feishuBaseUrl(authConfig), path, { ...requestOptions, signal, accessToken });
      return { payload, config: { ...authConfig, user_access_token: accessToken, forceUserAccessToken: 'true' }, authorizedAsUser: true };
    } catch (userErr) {
      if (shouldStartUserAuthorization(userErr)) {
        const { tokenData } = await authorizeAndWait(authConfig, { signal, emit, action: authAction });
        const payload = await requestJson(feishuBaseUrl(authConfig), path, { ...requestOptions, signal, accessToken: tokenData.accessToken });
        return { payload, config: { ...authConfig, user_access_token: tokenData.accessToken, forceUserAccessToken: 'true' }, authorizedAsUser: true };
      }
      throw userErr;
    }
  }
...
async function feishuResourcePayload(config, path, options = {}) {
  const result = await feishuResourceApi(config, path, options);
...
async function readDoc(input, { config, signal, emit }) {
  ...
  const payload = await feishuResourcePayload(config, `/open-apis/docx/v1/documents/${encodeURIComponent(documentId)}/raw_content`, {
    query: { lang },
    signal,
    emit,
    authAction: 'read_doc',
...
async function readOldDoc(input, { config, signal, emit }) {
  ...
  const payload = await feishuResourcePayload(config, `/open-apis/doc/v2/${encodeURIComponent(docToken)}/raw_content`, {
    signal,
    emit,
    authAction: 'read_old_doc',
    authDomains: ['docs'],
  });
  const maxTextChars = configuredInteger(config, 'maxTextChars', DEFAULT_MAX_TEXT_CHARS, 1000, 200_000);
...
async function getWikiNode(input, { config, signal, emit }) {
  ...
  const apiResult = await feishuResourceApi(config, '/open-apis/wiki/v2/spaces/get_node', {
    query: {
      token,
    ...
    signal,
    emit,
    authAction: 'get_wiki_node',
    authDomains: ['wiki'],
  });
  const payload = apiResult?.payload || apiResult;
  ...
  return {
    ...
    wikiToken: token,
    node,
    ...(apiResult?.authorizedAsUser ? { authorizedAsUser: true, authorizedConfig: apiResult.config } : {}),
  };
}
...
async function readWiki(input, ctx) {
  ...
  const readCtx = wiki.authorizedConfig ? { ...ctx, config: wiki.authorizedConfig } : ctx;

  if (objType === 'docx') {
    const doc = await readDoc({ ...input, documentId: objToken, url: undefined }, readCtx);
    return {
      ...doc,
      action: 'read_wiki',
      resourceType: 'wiki',
  ...
  if (objType === 'doc') {
    const doc = await readOldDoc({ ...input, docToken: objToken, url: undefined }, readCtx);
    return {
      ...doc,
      action: 'read_wiki',
      resourceType: 'wiki',
  ...
  if (objType === 'sheet') {
    const sheet = await readSheet({ ...input, spreadsheetToken: objToken, url: undefined }, readCtx);
    return {
      ...sheet,
      action: 'read_wiki',
      resourceType: 'wiki',
...
async function getSheetMeta(input, { config, signal, emit }) {
  ...
  const payload = await feishuResourcePayload(config, `/open-apis/sheets/v2/spreadsheets/${encodeURIComponent(spreadsheetToken)}/metainfo`, {
    query: {
      user_id_type: input.userIdType || configuredString(config, 'userIdType', 'open_id'),
    },
    signal,
    emit,
    authAction: 'get_sheet_meta',
    authDomains: ['sheets'],
  });
...
async function getCurrentUser(input, { config, signal, emit }) {
  const authConfig = await loadFeishuAuthConfig(config || {}, { includeStoredAuth: false });
  let payload;
  try {
    payload = await feishuUserApi(authConfig, '/open-apis/authen/v1/user_info', { signal });
  } catch (err) {
    if (configuredBoolean(authConfig, 'autoAuthorizeUser', true) && shouldStartUserAuthorization(err)) {
      const { tokenData } = await authorizeAndWait(authConfig, { signal, emit, action: 'get_current_user' });
      payload = await requestJson(feishuBaseUrl(authConfig), '/open-apis/authen/v1/user_info', {
        signal,
        accessToken: tokenData.accessToken,
      });
    } else {
...
async function readSheet(input, { config, signal, emit }) {
  ...
  const payload = await feishuResourcePayload(config, `/open-apis/sheets/v2/spreadsheets/${encodeURIComponent(spreadsheetToken)}/values_batch_get`, {
    query: {
      ranges,
...
export const feishuReadTool = {
  ...
  description: [
    ...
    '- To read the current authorized user, use action="get_current_user" with no userId. If no valid user_access_token is configured, the tool delegates to the shared feishu_auth Device Flow and waits for approval.',
    '- If reading docs/wiki/sheets fails with tenant permission errors, the tool requests the matching user OAuth scopes and retries with user_access_token, similar to lark-cli domain authorization.',
    '- For explicit authorization/login requests, use the feishu_auth tool. authorize_current_user and complete_current_user_authorization are kept only for compatibility.',
  ...
  systemPrompt() {
    return [
      ...
      '- get_current_user handles missing, invalid, or expired user_access_token by using the shared feishu_auth Device Flow inside the same tool call. Do not call ask_user for Feishu authorization and do not manually print verification steps unless the tool itself fails.',
      '- For document, wiki, or sheet permission errors, continue waiting for feishu_read. It can request resource read scopes through the same Feishu authorization UI and retry automatically.',
  ...
  configSchema: {
    ...
    properties: {
      ...
      oauthScope: { type: 'string', description: 'OAuth scopes requested by shared feishu_auth Device Flow. Resource reads add docs/wiki/sheets read scopes as needed.' },
      ...
      autoAuthorizeResources: { type: 'boolean', description: 'When reading docs/wiki/sheets with tenant permission errors, authorize user read scopes and retry with user_access_token.' },
  ...
  async handler(input, ctx) {
    switch (input.action) {
      ...
      case 'authorize_current_user':
        return startDeviceAuthorization(await loadFeishuAuthConfig(ctx.config || {}, { includeStoredAuth: false }));
      case 'complete_current_user_authorization':
        return completeDeviceAuthorization(input, {
          config: await loadFeishuAuthConfig(ctx.config || {}, { includeStoredAuth: false }),
          signal: ctx.signal,
        });
      default:
