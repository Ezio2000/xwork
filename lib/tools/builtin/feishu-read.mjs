...
async function completeUserAuthorization(input, { config, signal }) {
  const appId = configuredString(config, 'app_id', configuredString(config, 'appId', process.env.FEISHU_APP_ID || process.env.LARK_APP_ID));
  const appSecret = configuredString(config, 'app_secret', configuredString(config, 'appSecret', process.env.FEISHU_APP_SECRET || process.env.LARK_APP_SECRET));
  if (!appId || !appSecret) throw new Error('Feishu app_id/app_secret are required to complete device authorization.');
  const deviceCode = requiredToken(input.deviceCode || input.device_code, 'deviceCode');
  const payload = await requestJson(normalizeBaseUrl(configuredString(config, 'baseUrl', process.env.FEISHU_BASE_URL || DEFAULT_BASE_URL)), '/open-apis/authen/v2/oauth/token', {
    method: 'POST',
    body: {
  ...
  await patchStoredToolConfig('feishu_read', currentConfig => ({
    ...
    user_access_token_expires_at: data.expires_in
      ? new Date(Date.now() + Number(data.expires_in) * 1000).toISOString()
      : '',
    refresh_token: data.refresh_token || currentConfig.refresh_token || '',
...
async function getCurrentUser(input, { config, signal, context }) {
  let payload;
  try {
    payload = await feishuUserApi(config, '/open-apis/authen/v1/user_info', { signal });
  } catch (err) {
    if (configuredBoolean(config, 'autoAuthorizeUser', true) && /user_access_token is not configured/i.test(err.message || '')) {
      return createUserAuthorizationRequest(config);
    }
    throw err;
  }
  return {
...
export const feishuReadTool = {
  ...
  systemPrompt() {
    return [
      ...
      '- If get_current_user returns authorizationRequired=true, show the verificationUrl to the user. After they approve Feishu access, call action="complete_current_user_authorization" with the returned deviceCode, then retry get_current_user.',
