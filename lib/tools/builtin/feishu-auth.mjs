...
export const feishuAuthTool = {
  ...
  description: [
    ...
    '',
    'Actions:',
    '- login: start Device Flow, emit an authorization popup event, wait until the user approves, then save user_access_token.',
  ...
  defaultConfig: {
    ...
    authBaseUrl: DEFAULT_FEISHU_AUTH_BASE_URL,
    app_id: '',
    app_secret: '',
    user_access_token: '',
    oauthScope: DEFAULT_FEISHU_OAUTH_SCOPE,
  },
  configSchema: {
    type: 'object',
    properties: {
  ...
  async handler(input, ctx) {
    const config = await loadFeishuAuthConfig(ctx.config || {});
    switch (input.action) {
      case 'login': {
