...
function looksLikeHtml(text) {
  return /^\s*(?:<!doctype\s+html\b|<html[\s>]|<head[\s>]|<body[\s>])/i.test(String(text || ''));
}

function shouldConvertHtml(contentType, rawText) {
  const type = String(contentType || '').toLowerCase();
  return type.includes('text/html') || type.includes('application/xhtml') || looksLikeHtml(rawText);
}

function normalizedProxyUrl(proxy) {
  ...
  try {
    ...
    throw new Error(`Invalid web_fetch proxy URL: ${value}`);
  }
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    throw new Error(`Invalid web_fetch proxy URL protocol: ${url.protocol || value}`);
  }
  return url.toString();
}
...
export const webFetchTool = {
  ...
  async handler({ url, prompt }, { config, signal }) {
    ...
    const proxy = normalizedProxyUrl(config?.proxy || '');
    ...
    let markdown = shouldConvertHtml(contentType, rawText)
