export const calculatorTool = {
  ...
  parseResult(output) {
    const isError = output.result && typeof output.result === 'object' && output.result.error;
    return {
      renderType: 'calc-result',
      data: {
        expression: output.expression,
        result: isError ? null : output.result,
        error: isError ? output.result.error : null,
      },
    };
  },
};
