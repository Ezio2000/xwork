...
function systemPrompt(tool = {}) {
  return [
    ...
    '',
    '## Turn budget',
    '- Expert agents default to a configurable turn budget from their profile. Each turn = one assistant response + tool results. Multiple parallel tool calls in one response count as one turn.',
    '- Plan objectives so the expert agent can finish within the turn budget. If more rounds are needed, set maxTurns up to 10.',
...
const SUBAGENT_MAX_TURNS_MAX = 10;
const SUBAGENT_DEFAULT_MAX_TURNS = 3;

function configuredDefaultMaxTurns(toolConfig = {}) {
  const n = Number(toolConfig.defaultMaxTurns);
  if (!Number.isFinite(n)) return undefined;
  return Math.min(SUBAGENT_MAX_TURNS_MAX, Math.max(SUBAGENT_MAX_TURNS_MIN, Math.round(n)));
}

function resolveSubagentMaxTurns(input, toolConfig) {
  if (input.maxTurns !== undefined) return input.maxTurns;
  return configuredDefaultMaxTurns(toolConfig);
}
...
export const delegateTaskTool = {
  ...
  timeoutMs: 125000,
  defaultConfig: {
    defaultMaxTurns: SUBAGENT_DEFAULT_MAX_TURNS,
  },
  configSchema: {
    type: 'object',
    properties: {
      defaultMaxTurns: {
        type: 'number',
        description: 'Fallback expert-agent tool-calling turn budget when both delegate_task input and expert profile omit maxTurns. Range 1-10.',
      },
    },
  ...
  configExamples: [
    {
      title: 'Longer subagent investigations',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      maxTurns: {
        type: 'number',
        description: 'Optional expert-agent tool-calling turn budget for this call. Overrides the selected expert profile. Maximum 10.',
      },
      timeoutMs: {
        type: 'number',
        description: 'Optional expert-agent timeout in milliseconds. Default comes from the selected expert profile; maximum 120000.',
      },
      maxOutputChars: {
        type: 'number',
  ...
  validate(input) {
    ...
    optionalNumber(input.maxTurns, 'maxTurns', { min: 1, max: SUBAGENT_MAX_TURNS_MAX });
  ...
  async handler(input, { config: toolConfig = {}, context, signal }) {
    ...
    return context.runSubagent({
      ...
      maxTurns: resolveSubagentMaxTurns(input, toolConfig),
