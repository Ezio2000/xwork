function systemPrompt() {
  return [
    ...
    '',
    '## Turn budget',
    '- Subagents default to a configurable turn budget (delegate_task tool config defaultMaxTurns, fallback 3; max 10). Each turn = one assistant response + tool results. Multiple parallel tool calls in one response count as one turn.',
    '- Plan objectives so the subagent can finish within the turn budget. If more rounds are needed, set maxTurns up to 10.',
...
export const delegateTaskTool = {
  ...
  configSchema: {
    ...
    properties: {
      defaultMaxTurns: {
        type: 'number',
        description: 'Default subagent tool-calling turn budget when delegate_task input omits maxTurns. Range 1-10.',
      },
    },
  ...
  inputSchema: {
    ...
    properties: {
      ...
      maxTurns: {
        type: 'number',
        description: 'Optional subagent tool-calling turn budget for this call. Overrides delegate_task tool config defaultMaxTurns. Maximum 10.',
      },
      timeoutMs: {
        type: 'number',
