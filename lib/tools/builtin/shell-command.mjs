...
export const shellCommandTool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      maxOutputChars: {
        type: 'number',
        description: 'Optional combined stdout/stderr character budget. Default 20000, minimum 1000, maximum 100000. Smaller positive values are raised to 1000.',
      },
      outputEncoding: {
        type: 'string',
  ...
  validate(input) {
    ...
    if (input.maxOutputChars !== undefined) {
      ...
      if (!Number.isInteger(n) || n <= 0 || n > MAX_OUTPUT_CHARS) {
        throw new Error(`maxOutputChars must be a positive integer no greater than ${MAX_OUTPUT_CHARS}`);
      }
    }
  ...
  async handler(input, { signal, emit }) {
    return new Promise((resolvePromise, reject) => {
      ...
      child.stdout?.on('data', chunk => {
        ...
        emit?.({
          stream: 'stdout',
          text: decodeOutput([Buffer.from(chunk)], input.outputEncoding),
        });
      });
