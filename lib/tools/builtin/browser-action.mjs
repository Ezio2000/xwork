...
const BROWSER_PREVIEW_META = Symbol('browserPreview');
...
function previewScreenshotFilename(context = {}, action = 'browser') {
  const callId = typeof context.toolCallId === 'string' && context.toolCallId.trim()
    ? context.toolCallId.trim()
    : `${Date.now()}-${randomUUID().slice(0, 8)}`;
  return screenshotFilename(`preview-${callId}-${action}`);
}

async function captureBrowserPreview(page, config = {}, context = {}, action = 'browser', timeoutMs = DEFAULT_ACTION_TIMEOUT_MS) {
  if (!page || page.isClosed?.()) return {};
  try {
    const dir = screenshotDir({ ...config, screenshotDir: DEFAULT_SCREENSHOT_DIR });
    await mkdir(dir, { recursive: true });
    const path = join(dir, previewScreenshotFilename(context, action));
    await page.screenshot({
      path,
      fullPage: false,
      timeout: Math.min(Math.max(Number(timeoutMs) || DEFAULT_ACTION_TIMEOUT_MS, 1000), 15_000),
    });
    return {
      previewScreenshotPath: path,
      previewScreenshotUrl: screenshotUrlFromPath(path),
...
function attachBrowserPreview(output, preview = {}) {
  if (!output || typeof output !== 'object') return output;
  Object.defineProperty(output, BROWSER_PREVIEW_META, {
    value: preview || {},
    enumerable: false,
    configurable: true,
  });
...
function browserPreviewFromOutput(output) {
  return (output && typeof output === 'object' && output[BROWSER_PREVIEW_META]) || {};
}
...
export const browserActionTool = {
  ...
  async handler(input, { config, context, signal, emit }) {
    ...
    const completeAction = async (output, event = {}, previewOverride = null) => {
      const preview = previewOverride || await captureBrowserPreview(page, config, context, action, actionTimeout);
      const outputWithPreview = attachBrowserPreview(output, preview);
      emitStep(emit, {
        phase: 'complete',
        action,
        url: output.url,
    ...
    if (action === 'screenshot') {
      ...
      const preview = {
        previewScreenshotPath: path,
        previewScreenshotUrl: output.screenshotUrl,
        previewCapturedAt: new Date().toISOString(),
      };
      return completeAction(output, { screenshotPath: path, screenshotUrl: output.screenshotUrl, fullPage: output.fullPage }, preview);
    }
  ...
  parseResult(output) {
    const preview = browserPreviewFromOutput(output);
    return {
      ...
      data: {
        ...
        previewScreenshotPath: preview.previewScreenshotPath,
        previewScreenshotUrl: preview.previewScreenshotUrl,
        previewCapturedAt: preview.previewCapturedAt,
