...
function configuredInteger(config, key, fallback, { min = 1, max = 310000 } = {}) {
...
function configuredToolDefinition(tool, config = {}, runtimeContext = {}) {
  const toolConfig = plainObject(config.config);
  const definition = {
    name: tool.name,
    description: tool.description,
  ...
  // Let the tool rewrite its own API-facing definition with runtime data it
  // declared via runtimeContext (e.g. delegate_task injects the expert catalog).
  // The framework stays tool-name agnostic.
  if (typeof tool.resolveDefinition === 'function') {
    return tool.resolveDefinition(definition, runtimeContext) || definition;
  }
  return definition;
}

// Lazily collect only the runtime-context keys declared by the enabled tools, so
// expensive lookups (e.g. listing expert agents) run only when a tool needs them.
const RUNTIME_CONTEXT_LOADERS = {
  expertAgents: () => listEnabledExpertAgentsForPrompt(),
};

async function collectRuntimeContext(tools) {
  const needed = new Set();
  for (const tool of tools) {
    for (const key of Array.isArray(tool.runtimeContext) ? tool.runtimeContext : []) {
      if (RUNTIME_CONTEXT_LOADERS[key]) needed.add(key);
    }
  }
  const context = {};
  await Promise.all([...needed].map(async key => {
    context[key] = await RUNTIME_CONTEXT_LOADERS[key]();
  }));
  return context;
}
...
export async function getEnabledToolDefinitions() {
  ...
  const configById = new Map(configs.map(config => [config.id, config]));
  const enabledTools = tools.filter(tool => enabledIds.has(tool.id) && tool.adapter !== 'unavailable');
  const runtimeContext = await collectRuntimeContext(enabledTools);
  return enabledTools
    .map(tool => configuredToolDefinition(tool, configById.get(tool.id), runtimeContext));
}
