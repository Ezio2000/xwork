...
export async function runTool(call, context = {}) {
  ...
  if (!isError && typeof tool.modelOutput === 'function') {
    try {
      publicOutput = await tool.modelOutput(output, handlerInput, hookCtx);
    } catch {}
  }

  const result = { id: call.id, name: call.name, isError, output: publicOutput, durationMs };

  // parseResult — convert model-visible output to frontend render metadata.
  let render;
  if (!isError && typeof tool.parseResult === 'function') {
    try {
      render = tool.parseResult(publicOutput, handlerInput);
    } catch {}
  }
