...
import { formatToolOutput, runTool } from '../runner.mjs';
...
async function withFeishuReadEnabled(config, fn) {
  ...
  try {
    ...
    await updateToolConfig('feishu_read', {
      ...
      timeoutMs: current?.timeoutMs ?? tool.timeoutMs,
      config: current?.config || tool.defaultConfig,
    });
    await updateToolConfig('feishu_auth', {
      enabled: currentAuth?.enabled ?? false,
      timeoutMs: currentAuth?.timeoutMs ?? 300000,
...
describe('feishu_read tool', () => {
  ...
  it('extracts tokens from Feishu document and sheet URLs', () => {
    assert.deepEqual(
      tool.__test.extractTokenFromUrl('https://example.feishu.cn/docx/AbCdEfGhIjKl', 'doc'),
      { documentId: 'AbCdEfGhIjKl', wikiToken: undefined },
    );
    assert.deepEqual(
      tool.__test.extractTokenFromUrl('https://example.feishu.cn/sheets/shtcnToken?sheet=abc123', 'sheet'),
      { spreadsheetToken: 'shtcnToken', sheetId: 'abc123' },
    );
    assert.deepEqual(
      tool.__test.extractTokenFromUrl('https://example.feishu.cn/wiki/I6BQwED7mi3x1Ek0WimcNLSZnIh', 'wiki'),
      { wikiToken: 'I6BQwED7mi3x1Ek0WimcNLSZnIh', documentId: undefined, docToken: undefined, spreadsheetToken: undefined },
    );
  });

  it('accepts the compact media listing action', () => {
    assert.doesNotThrow(() => tool.validate({ action: 'list_doc_media', documentId: 'docx_token' }));
  });

  it('compacts large docx block outputs before they reach model context', () => {
    const raw = {
      action: 'get_doc_blocks',
      resourceType: 'docx',
      documentId: 'docx_token',
      blockCount: 2,
      blocks: [
        { block_id: 'title_1', block_type: 3, heading1: { elements: [{ text_run: { content: 'Overview' } }] } },
        { block_id: 'image_1', block_type: 27, image: { token: 'img_token_1', width: 640, height: 320 } },
      ],
      tree: [{ block_id: 'title_1', children: [{ block_id: 'image_1' }] }],
      hasMore: false,
    };

    const output = tool.modelOutput(raw, { action: 'get_doc_blocks' }, { config: {} });

    assert.equal(output.action, 'get_doc_blocks');
    assert.equal(output.blockCount, 2);
    assert.equal(output.assets[0].token, 'img_token_1');
    assert.equal(output.assets[0].width, 640);
    assert.equal(output.blockPreview.some(block => block.blockId === 'image_1'), true);
    assert.equal(output.blocks, undefined);
    assert.equal(output.tree, undefined);
    assert.ok(JSON.stringify(output).length < 5000);
  });

  it('renders Feishu snippets from preview data without storing full content in the UI block', () => {
    const output = {
      ...
      content: '# Title\n\nBody',
      contentLength: 12,
      assets: [{ token: 'img_token_1' }],
    };

    const render = tool.parseResult(output);

    assert.equal(render.renderType, 'file-snippet');
    assert.equal(render.data.content, undefined);
    assert.equal(render.data.contentPreview, '# Title\n\nBody');
    assert.equal(render.data.contentFormat, 'markdown');
  });

  it('lists docx media with compact output only', async () => {
    ...
    globalThis.fetch = async (url, options) => {
      ...
      return jsonResponse({
        ...
        data: {
          has_more: false,
          items: [
            { block_id: 'text_1', block_type: 2, text: { elements: [{ text_run: { content: 'Intro' } }] } },
            { block_id: 'image_1', block_type: 27, image: { token: 'img_token_1', width: 640, height: 320 } },
          ],
        },
      });
    ...
    try {
      const output = await tool.handler(
        { action: 'list_doc_media', documentId: 'docx_token' },
        { config: { user_access_token: 'u-docx-token' } },
      );

      assert.equal(output.action, 'list_doc_media');
      assert.equal(output.blockCount, 2);
      assert.equal(output.assetCount, 1);
      assert.equal(output.assets[0].token, 'img_token_1');
      assert.equal(output.assets[0].width, 640);
      assert.equal(output.blocks, undefined);
      assert.equal(output.tree, undefined);
  ...
  it('reads docx blocks with media assets and tree structure', WRITES_REAL_TOOL_DB_SKIP, async () => {
    ...
    try {
      await withFeishuReadEnabled({ user_access_token: 'u-docx-token' }, async () => {
        ...
        assert.equal(result.output.blockPreview.some(block => block.blockId === 'image_1'), true);
        assert.equal(result.output.tree, undefined);
        assert.equal(result.output.blocks, undefined);
        assert.match(calls[0].url.pathname, /\/docx\/v1\/documents\/docx_token\/blocks$/);
  ...
  it('downloads Feishu media to a local preview file', WRITES_REAL_TOOL_DB_SKIP, async () => {
    ...
    try {
      await withFeishuReadEnabled({ user_access_token: 'u-media-token' }, async () => {
        ...
        assert.match(modelVisible, /Do not call shell_command/);
  ...
  it('gets Feishu user info with user id type options', WRITES_REAL_TOOL_DB_SKIP, async () => {
    ...
    try {
      await withFeishuReadEnabled({ user_access_token: 'u-user-token' }, async () => {
        ...
        assert.equal(result.render.renderType, 'file-snippet');
        assert.match(result.render.data.contentPreview, /Alice/);
      });
    } finally {
      globalThis.fetch = previousFetch;
    }
  ...
  it('gets current authorized Feishu user without requiring a user id', WRITES_REAL_TOOL_DB_SKIP, async () => {
    ...
    try {
      await withFeishuReadEnabled({ user_access_token: 'u-current' }, async () => {
        ...
        assert.match(result.render.data.contentPreview, /Current User/);
      });
    } finally {
      globalThis.fetch = previousFetch;
    }
  ...
  it('starts device authorization explicitly', WRITES_REAL_TOOL_DB_SKIP, async () => {
    ...
    await withFeishuReadEnabled({
      ...
      assert.match(result.render.data.contentPreview, /authorizationUrl/);
    });
    globalThis.fetch = previousFetch;
  });
