...
async function getDocBlocks(input, { config, signal, emit }) {
  if (isWikiUrl(input.url)) {
    ...
    const result = await getDocBlocks({ ...input, documentId: wiki.node.obj_token, url: undefined }, { config, signal, emit });
    return {
      ...result,
      resourceType: 'wiki',
...
async function readDocRich(input, ctx) {
  const result = await getDocBlocks(input, ctx);
...
export const tool = {
  ...
  title: 'Feishu Read',
  description: [
    'Read Feishu/Lark cloud documents, spreadsheets, media assets, and user profile data using Feishu OpenAPI with user authorization. Supports new Docx raw text/block tree/rich structured content, legacy Doc raw text, spreadsheet metadata/range values, media downloads, contact user lookup, and current authorized user lookup.',
    '',
    ...
    '- For Docx block structure use action="get_doc_blocks"; for structured rich content with media references use action="read_doc_rich".',
  ...
  systemPrompt() {
    return [
      ...
      '- When the user asks for document images, attachments, or exact Docx structure, call action="read_doc_rich" or action="get_doc_blocks" instead of read_doc.',
      ...
      '- get_doc_blocks/read_doc_rich require docs scopes; download_media requires docs plus media download scope. The authorization flow appends these scopes automatically.',
  ...
  configSchema: {
    ...
    properties: {
      ...
      maxBlocks: { type: 'number', description: 'Maximum Docx blocks returned by get_doc_blocks/read_doc_rich.' },
      pageSize: { type: 'number', description: 'Docx block page size.' },
  ...
  inputSchema: {
    ...
    properties: {
      action: {
        type: 'string',
        description: 'Operation to perform: read_doc, get_doc_blocks, read_doc_rich, download_media, read_old_doc, read_wiki, get_wiki_node, read_sheet, get_sheet_meta, get_user, get_current_user, authorize_current_user, or complete_current_user_authorization.',
      },
      url: {
        type: 'string',
      ...
      pageSize: {
        type: 'number',
        description: 'Docx blocks page size.',
      },
      maxBlocks: {
        type: 'number',
        description: 'Maximum Docx blocks to return.',
      },
      includeTree: {
        type: 'boolean',
        description: 'Whether get_doc_blocks/read_doc_rich should include a parent/child block tree.',
      },
      fileToken: {
        type: 'string',
  ...
  parseResult(output) {
    ...
    const content = output.content || JSON.stringify(output.resourceType === 'media'
      ? {
      ...
      ? { user: output.user }
      : output.resourceType === 'wiki'
        ? { node: output.node, blockCount: output.blockCount, assets: output.assets, blocks: output.blocks }
      : output.resourceType === 'authorization'
        ? {
      ...
      : output.resourceType === 'docx'
        ? { documentId: output.documentId, blockCount: output.blockCount, assets: output.assets, blocks: output.blocks }
