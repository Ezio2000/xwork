...
function modelVisibleFeishuOutput(output, input, { config } = {}) {
  ...
  if (output.resourceType === 'media') {
    return compactObject({
      ...
      nextStep: 'Do not call shell_command, write_file, or read_file to display this media again; summarize briefly to the user.',
    });
  }
  if (
...
export const tool = {
  ...
  description: [
    ...
    '- After download_media succeeds for an image, the image is already shown to the user by the UI. Do not call shell_command, write_file, read_file, or create HTML just to display it again.',
  ...
  systemPrompt() {
    return [
      ...
      '- After a successful download_media call, stop using tools for display. Do not call shell_command, write_file, read_file, or create local HTML to show the same image.',
