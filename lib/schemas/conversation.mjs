...
import { validateChannelPayload, validateVisionConfig } from './channel.mjs';
import { validateChannelPricing } from './pricing.mjs';
import { legacyVisionProviderToProvider, normalizeVisionProvider } from './vision-provider.mjs';
...
function normalizeVisionProviders(cfg, vision) {
  const providers = [];
  const seen = new Set();
  const addProvider = (provider) => {
    const normalized = normalizeVisionProvider(provider);
    if (!normalized || seen.has(normalized.id)) return null;
    seen.add(normalized.id);
    providers.push(normalized);
    return normalized;
  };

  for (const provider of Array.isArray(cfg?.visionProviders) ? cfg.visionProviders : []) {
    addProvider(provider);
  }

  if (vision?.defaultProvider && !vision.defaultProviderId) {
    const migrated = legacyVisionProviderToProvider(vision.defaultProvider, 'default-vision-provider');
    const provider = addProvider(migrated);
    if (provider) vision.defaultProviderId = provider.id;
  }

  if (vision?.defaultChannelId && vision.defaultModelId && !vision.defaultProviderId) {
  ...
  if (vision?.defaultProviderId && !providers.some(provider => provider.id === vision.defaultProviderId)) {
    vision.defaultProviderId = null;
  }
...
export function normalizeAppConfig(cfg) {
  ...
  const vision = validateVisionConfig(cfg?.vision);
  const visionProviders = normalizeVisionProviders(cfg, vision);
  return { channels, activeChannelId, activeModel, workspace, vision, visionProviders };
}
