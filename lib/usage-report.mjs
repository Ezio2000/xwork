...
function toolCounts(run) {
  ...
  for (const event of run?.events || []) {
    ...
    if (type === AGENT_EVENT_TYPES.TOOL_CALL || type === AGENT_EVENT_TYPES.SUBAGENT_TOOL_CALL) counts.localToolCalls++;
    if (type === 'server_tool_call') counts.serverToolCalls++;
    if (type === AGENT_EVENT_TYPES.SUBAGENT_SERVER_TOOL && event.event?.phase === 'call') counts.serverToolCalls++;
    if (type === AGENT_EVENT_TYPES.TOOL_RESULT || type === AGENT_EVENT_TYPES.SUBAGENT_TOOL_RESULT || type === 'server_tool_result') counts.toolResults++;
    if (type === AGENT_EVENT_TYPES.SUBAGENT_SERVER_TOOL && event.event?.phase === 'result') counts.toolResults++;
  }
...
function addCostSummary(target, itemCost) {
  ...
  addOptionalCost(target, 'requestCost', itemCost.requestCost);
