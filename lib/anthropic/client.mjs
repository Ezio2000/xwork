...
const DSML_TOOL_CALL_END = '</||DSML||tool_calls>';
const DSML_MARKER_TAIL_LENGTH = Math.max(DSML_TOOL_CALL_START.length, DSML_TOOL_CALL_END.length) - 1;
...
function processVisibleText(filter, text, { final = false } = {}) {
  ...
  while (filter.buffer) {
    if (filter.inside) {
      const endIdx = filter.buffer.indexOf(DSML_TOOL_CALL_END);
      if (endIdx === -1) {
        filter.buffer = final ? '' : filter.buffer.slice(-DSML_MARKER_TAIL_LENGTH);
        return visible;
      }
      filter.buffer = filter.buffer.slice(endIdx + DSML_TOOL_CALL_END.length);
      filter.inside = false;
      continue;
    }

    const startIdx = filter.buffer.indexOf(DSML_TOOL_CALL_START);
    if (startIdx !== -1) {
      visible += filter.buffer.slice(0, startIdx);
      filter.buffer = filter.buffer.slice(startIdx + DSML_TOOL_CALL_START.length);
      filter.inside = true;
      continue;
    }
