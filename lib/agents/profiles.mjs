...
import {
  RETIRED_BUILTIN_EXPERT_AGENT_ALIASES,
  RETIRED_BUILTIN_EXPERT_AGENT_IDS,
  builtInScenarioAgents,
...
function mergeBuiltIns(profiles = []) {
  ...
  const retiredBuiltIns = new Set(RETIRED_BUILTIN_EXPERT_AGENT_IDS);
  const compactedProfiles = profiles.filter(profile => {
    if (!retiredBuiltIns.has(profile?.id)) return true;
    return profile.builtin !== true;
  });
  const removedRetiredBuiltIns = compactedProfiles.length !== profiles.length;
  if (removedRetiredBuiltIns) profiles = compactedProfiles;
  const byId = new Map();
  for (const profile of profiles) byId.set(profile.id, profile);
  let changed = removedRetiredBuiltIns;
...
export async function resolveExpertAgentForRun(id) {
  ...
  const resolvedId = RETIRED_BUILTIN_EXPERT_AGENT_ALIASES[requestedId] || requestedId;
  const profiles = await readProfiles();
  const enabledToolNames = await mainAgentEnabledToolNameSet();
  const requested = profiles.find(profile => profile.id === resolvedId);
