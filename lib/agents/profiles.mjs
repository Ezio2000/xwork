...
function defaultData() {
  return { profiles: [builtInGeneralAgent()] };
}
...
function mergeBuiltIns(profiles = []) {
  const builtIns = [builtInGeneralAgent()];
...
export async function resolveExpertAgentForRun(id) {
  ...
  if (!requested) {
    return {
      profile: publicProfile(fallback),
      requestedId,
  ...
  return { profile: publicProfile(requested), requestedId, fallbackReason: '' };
}
