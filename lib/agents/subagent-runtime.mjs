...
function buildSubagentHistory({ objective, task, brief, expectedOutput, instructions, parentSummary, allowedToolNames, allowSubagents, maxTurns }) {
  ...
  const systemParts = [
    ...
    'Your entire output is pasted verbatim into the parent agent. Every extra word costs the parent tokens, so be ruthlessly concise.',
    'Output ONLY raw findings — no introductions ("I searched for..."), no meta-commentary ("Based on the results..."), no transition phrases, no summaries. Just state what you found.',
    'Use as few words as possible while preserving accuracy. Prefer a few bullet points over paragraphs. If a one-line answer suffices, stop there.',
    ...
    'Expected output: the parent needs raw facts, not prose. 3-6 bullets max. No greetings, no sign-offs, no commentary.',
