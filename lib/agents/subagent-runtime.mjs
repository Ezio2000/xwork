...
function resolveAllowedToolNames({ profile, allowedTools, allowSubagents }) {
  const profileTools = Array.isArray(profile?.allowedTools) && profile.allowedTools.length
  ...
  } else if (profileTools.includes('delegate_task') || allowedTools?.includes?.('delegate_task')) {
...
async function resolveProfileChannelConfig({ config, profile }) {
  ...
  try {
    ...
    const requestModel = model || appConfig.activeModel || channel.models?.[0] || config?.model || '';
    return {
      config: {
        ...(config || {}),
