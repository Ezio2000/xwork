...
export async function runChatRequest({ payload, signal, emit, rootRunId }) {
  const { conversationId, message, images = [], channelId, model } = payload;
  let rootContext = null;

  try {
    const resolveConfig = CHAT_SERVICE_TEST_HOOKS.resolveRuntimeChannelConfig || resolveRuntimeChannelConfig;
    const resolved = await resolveConfig({ channelId, model });
    if (resolved.error) {
      emit(errorEvent(resolved.error));
      return;
    }

    const { channel, requestModel, modelConfig, channelConfig, cfg } = resolved;
    const { history, replayHistory, existingTitle, originalMessageCount } = await loadConversationState(conversationId);
    const turnStartIndex = await appendUserMessage(history, message, images);
    const replayTurnStartIndex = replayHistory.length;
    const replayUserContent = history[turnStartIndex]?.content ?? message;
    replayHistory.push({ role: 'user', content: replayUserContent });
    ...
    const replayWithImagePolicy = await projectImagesForModel(replayHistory, {
      modelConfig,
      appConfig: cfg,
      emit,
      userInputRegistry: globalUserInputRegistry,
    ...
    const historyForApi = await expandFileMentionsInHistory(replayWithImagePolicy);

    const toolBudgetGuard = createToolBudgetGuard(channelConfig.tools);
    ...
    while (!iterResult.done) {
      ...
      if (evt.type === RUN_EVENT_TYPES.TOOL_CALL) {
        ...
        emit(evt);
      } else if (evt.type === RUN_EVENT_TYPES.ASSISTANT_RETRY) {
        emit(assistantRetryEvent(evt.reason, evt.message));
    ...
    const { agentRuns, trace } = await rootContext.completeAndBuildTrace(finalState, { turnStartIndex: replayTurnStartIndex });
