...
const DEFAULT_MAX_CONCURRENT_DELEGATES = 3;

function maxConcurrentDelegates(toolContext) {
  const value = Number(toolContext?.maxConcurrentSubagents);
  if (!Number.isFinite(value)) return DEFAULT_MAX_CONCURRENT_DELEGATES;
  ...
  while (true) {
    ...
    for (let i = 0; i < result.toolCalls.length;) {
      ...
      if (call.name === DELEGATE_TASK_TOOL_NAME) {
        const maxConcurrent = maxConcurrentDelegates(toolContext);
        const batch = [];
        while (i < result.toolCalls.length && result.toolCalls[i].name === DELEGATE_TASK_TOOL_NAME && batch.length < maxConcurrent) {
