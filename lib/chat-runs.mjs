...
import { writeSse, writeSseComment, writeSseDone } from './sse-writer.mjs';
...
const subscriberHeartbeats = new WeakMap();
...
function safeWriteHeartbeat(res) {
  if (!res || res.destroyed || res.writableEnded) return false;
  try {
    writeSseComment(res);
    return true;
...
function removeSubscriber(run, res) {
  const heartbeat = subscriberHeartbeats.get(res);
  if (heartbeat) {
    clearInterval(heartbeat);
    subscriberHeartbeats.delete(res);
  }
  run?.subscribers?.delete(res);
}

function attachHeartbeat(run, res, { heartbeatMs = DEFAULT_SSE_HEARTBEAT_MS } = {}) {
  if (!res || heartbeatMs <= 0 || subscriberHeartbeats.has(res)) return;
  const heartbeat = setInterval(() => {
    if (!safeWriteHeartbeat(res)) removeSubscriber(run, res);
  }, heartbeatMs);
  heartbeat.unref?.();
  subscriberHeartbeats.set(res, heartbeat);
}
...
export function subscribeResponseToRun(run, res, { afterSeq = 0, heartbeatMs = DEFAULT_SSE_HEARTBEAT_MS } = {}) {
  ...
  attachHeartbeat(run, res, { heartbeatMs });
  res.on('close', () => {
    removeSubscriber(run, res);
  });
}
