...
function buildBlocksFromResult(result) {
  ...
  // Build server tool event lookup
  const resultByCallId = {};
  for (const event of (result.serverToolEvents || [])) {
    if (event.phase === 'result') resultByCallId[event.id] = event;
  }

  // Build builtin tool result lookup by tool_use_id
      ...
      if (part.type === 'tool_result' && typeof part.content === 'string') {
        try {
          ...
          if (Array.isArray(data.uuids)) {
            toolResultById[part.tool_use_id] = {
              type: 'uuid-list',
              uuids: data.uuids,
              count: data.count ?? data.uuids.length,
            };
          }
        } catch {}
      }
  ...
  // Process ALL assistant messages in order to build blocks in correct sequence
  for (const msg of msgs) {
    ...
    for (const part of msg.content) {
      if (part.type === 'text') {
        ...
        const resEvent = resultByCallId[id];
