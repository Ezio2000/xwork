...
function buildSystemPrompt(messages, config) {
  const parts = [
    [
...
function buildBlocksFromResult(result) {
  ...
  // Unified render lookup: maps tool_use_id → block { type, data fields... }
  const renderByCallId = {};
  for (const event of (result.serverToolEvents || [])) {
    if (event.phase === 'result' && event.renderType && event.data) {
      renderByCallId[event.id] = { type: event.renderType, ...event.data };
    }
  }
  for (const r of (result.builtinToolResults || [])) {
    if (r.renderType && r.data) {
      renderByCallId[r.callId] = { type: r.renderType, ...r.data };
    }
  }
...
export function assistantMessage(result, model) {
  ...
  const searchCount = (result.serverToolEvents || []).reduce((sum, e) => {
    if (e.phase === 'result' && e.data?.searchCount) return sum + e.data.searchCount;
    return sum;
  }, 0);
