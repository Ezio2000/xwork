...
function buildBlocksFromResult(result) {
  ...
  // Collect builtin tool render data from tool result messages
  const toolResultData = [];
  for (const msg of (result.__toolResults || [])) {
        ...
        if (part.type === 'tool_result' && typeof part.content === 'string') {
          try {
            ...
            if (Array.isArray(data.uuids)) {
              toolResultData.push({ renderType: 'uuid-list', data: { uuids: data.uuids, count: data.count ?? data.uuids.length } });
            }
          } catch {}
        }
      }
  ...
  // Append builtin tool render blocks after the last text block
  for (const tr of toolResultData) {
    if (tr.renderType === 'uuid-list' && tr.data?.uuids?.length) {
      blocks.push({ type: 'uuid-list', uuids: tr.data.uuids, count: tr.data.count });
      blocks.push({ type: 'text', content: '' });
    }
  }
